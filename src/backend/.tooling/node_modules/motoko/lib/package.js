...
async function fetchPackageFiles(info) {
    ...
    if (possiblyCDN) {
        try {
            const result = await fetchFromService(info, 'CDN', `https://data.jsdelivr.com/v1/package/gh/${repoPart}@${info.branch}/flat`, `https://cdn.jsdelivr.net/gh/${repoPart}@${info.branch}`, 'files', 'name');
            if (result) {
                return result;
            }
        }
    ...
    return await fetchFromService(info, 'GitHub', `https://api.github.com/repos/${repoPart}/git/trees/${info.branch}?recursive=1`, `https://raw.githubusercontent.com/${repoPart}/${info.branch}/`, 'tree', 'path', (file) => file.type === 'blob');
}
async function fetchFromService(info, serviceName, metaUrl, baseUrl, resultProperty, pathProperty, condition) {
    const response = await (0, cross_fetch_1.default)(metaUrl);
    if (!response.ok) {
    ...
    let directory = info.dir
        ...
        : '';
    const files = {};
    await Promise.all(json[resultProperty]
        .filter((file) => {
        return ((!directory ||
        ...
        .map(async (file) => {
        const response = await (0, cross_fetch_1.default)(`${baseUrl}${file[pathProperty]}`);
        if (!response.ok) {
            throw Error(response.statusText);
        }
...
async function fetchPackage(name, info) {
...
async function installPackages(mo, packages) {
    await Promise.all(Object.entries(packages).map(async ([name, info]) => {
