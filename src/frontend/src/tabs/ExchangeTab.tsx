...
interface CatalogTabProps {
	actor: ExchangeCatalogActor | null;
	isAuthenticated: boolean;
	onOpenPair: (pair: TokenCode) => void;
}

interface CatalogPairRecord {
	pairId: string;
	baseToken: string;
	quoteToken: string;
...
interface ExchangeCatalogActor {
	getTradingPairs?: () => Promise<[string, CatalogPairRecord][]>;
	getAllCustomTokens?: () => Promise<CatalogTokenRecord[]>;
	getTradeableArtifacts?: () => Promise<CatalogArtifactRecord[]>;
	createCustomToken?: (
		symbol: string,
		name: string,
		description: string,
	...
	verifyAndListToken?: (tokenId: string) => Promise<unknown>;
}

function CatalogTab({ actor, isAuthenticated, onOpenPair }: CatalogTabProps) {
	const [pairs, setPairs] = useState<[string, CatalogPairRecord][]>([]);
	const [tokens, setTokens] = useState<CatalogTokenRecord[]>([]);
	const [artifacts, setArtifacts] = useState<CatalogArtifactRecord[]>([]);
						...
						<table className="w-full">
							...
							<tbody>
								{pairs.map(([pairId, pair]) => {
									...
									const canOpen = ALL_TOKENS.some(
										(token) => token.code === baseToken,
									);
									return (
										<tr
											key={pairId}
										...
										<div className="flex items-center gap-2">
											...
											{!token.listedOnExchange && (
												<button
													...
													disabled={!isAuthenticated || submitting}
													onClick={() =>
														void handleVerifyAndList(token.tokenId)
													}
													className="font-mono text-[8px] tracking-[0.25em] px-2 py-1 border disabled:opacity-40"
													style={{ color: C.gold, borderColor: `${C.gold}33` }}
												>
...
export function ExchangeTab() {
	...
	return (
      ...
      {subTab === "CATALOG" && (
        <CatalogTab
          actor={actor as ExchangeCatalogActor | null}
          isAuthenticated={isAuthenticated}
          onOpenPair={(pair) => {
